...
/**
 * Client-side behaviour for the review-only "Kraft Riso" design: the scroll
 * reveals. Unlike Washed Chambray this design's nav has no scrolled state — it
 * is a printed band with a hard rule under it at every scroll position — so the
 * reveals are all there is.
 *
 * Everything is scoped to the .kraft-top subtree, and the whole subtree is
 ...
 */
export default function KraftBehaviour() {
  useEffect(() => {
    const root = document.querySelector<HTMLElement>(".kraft-top");
    if (!root) return;
